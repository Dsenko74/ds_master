...
const FinalizeForm = ({ delivery, setOrders }) => {
  const navigate = useNavigate();
  const getValidationSchema = (delivery) => {
    if (delivery) {
      // доставка — валідуємо адресу
      return Yup.object({
        privacyPolicy: Yup.boolean().oneOf([true], "Це обов’язкове поле"),
        street: Yup.string()
        ...
        apartment: Yup.number()
          ...
          .when("privateHome", {
            is: false,
            then: (schema) =>
              schema.required("Це обовязкове поле").min(1, "Не менше 1"),
  ...
  return (
    ...
    >
      <Form className="finalize__form">
        ...
        <FinalizePayment />
        <FinalizePrivacy />
        <FinalizeSubmitButton setOrders={setOrders} />
      </Form>
    </Formik>
  );
};
